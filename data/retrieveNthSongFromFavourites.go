...
func RetrieveNthSongFromFavourites(token string, n int) (*Track, error) {
	...
	if resp.StatusCode == 429 {
		...
		if err == nil {
			fmt.Printf("RetrieveNthSongFromFavourites rate limited, retry after: %s seconds\n", retryAfter)
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("Spotify API returned status %d when retrieving favourites", resp.StatusCode)
	}
